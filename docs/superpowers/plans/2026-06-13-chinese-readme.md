...
- [ ] Draft the document with sections for positioning, advantages, capability
...
- [ ] Use only placeholder secrets such as `${DEEPSEEK_API_KEY}` and sample IDs
  such as `123456789` or `222222222`.
- [ ] Avoid claiming capabilities not wired into `AppRuntime`.
...
- [ ] Add a short language navigation line near the title:
  `[中文说明](README.zh-CN.md)`.
- [ ] Leave the existing English quick start intact.
...
- [ ] Run a Markdown link/path check for `README.zh-CN.md` and `README.md`.
- [ ] Run a staged/document sensitive-info scan for API keys, local QQ IDs,
  WebUI tokens, NapCat login data paths, and local config file content.
- [ ] Run `git diff --check`.
- [ ] Optionally run `py -m pytest` only if non-doc files changed.
