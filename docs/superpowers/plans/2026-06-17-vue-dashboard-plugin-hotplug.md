...
Also add a regression test that two plugins with the same display `name` but
different sources do not share private KV data. The existing `plugin_kv` table can keep
its `plugin_name` column, but the value written there must be the stable plugin ID
such as `builtin:echo` or `workspace:echo`.
...
Also cover that `ctx.kv_set()` and `ctx.kv_get()` use the plugin ID, not just
the display name, so `builtin:echo` and `workspace:echo` keep separate KV values.
...
`kv_set()` and `kv_get()` must call `PluginKVStore` with the plugin ID as the
stored plugin key. Do not add a new KV table for this first pass; the existing
`plugin_kv.plugin_name` column stores the stable plugin ID.
...
- `reload()` calls `teardown(ctx)`, removes old tools/events, then loads updated plugin code.
...
Add `list_plugins()`, async `enable(source, name)`, async `disable(source, name)`,
async `reload(source, name)`, sync startup-only `load_enabled()`, and internal
cleanup helpers. Runtime hotplug methods must use a manager-level `asyncio.Lock`.
`load_all()` remains a synchronous startup compatibility wrapper and is only called
before the Dashboard starts accepting concurrent requests.
...
- `GET /api/plugins` returns `mode="standalone"`.
- plugin API routes require Dashboard auth when `access_token` is configured.
...
- the workspace plugin's `setup()` is not executed in standalone mode.
...
- reload calls teardown, removes old tools/events, and updates plugin code.
...
Do not commit `dashboard-ui/dist`; production deployments build it before running
the Python Dashboard. The Python backend still has a Chinese fallback when `dist`
is absent.
...
- [ ] **Step 6: Run repeatable frontend smoke**

Run a lightweight content check against the built files, for example:

```powershell
Get-ChildItem dashboard-ui/dist -Recurse -File |
  Select-String -Pattern "小助手控制台","总览","插件" -SimpleMatch
```

Expected: the built assets contain the core Chinese shell labels.

- [ ] **Step 7: Run Python static tests**
...
```bash
git add .gitignore dashboard-ui/package.json dashboard-ui/package-lock.json dashboard-ui/vite.config.ts dashboard-ui/tsconfig.json dashboard-ui/index.html dashboard-ui/src tests/test_dashboard.py
...
- [ ] **Step 6: Run repeatable frontend content check**

Run:

```powershell
Get-ChildItem dashboard-ui/dist -Recurse -File |
  Select-String -Pattern "记忆","会话","运行事件","主动推送","漂移任务" -SimpleMatch
```

Expected: built assets contain the Chinese view labels.
...
- [ ] **Step 6: Run repeatable plugin view content check**

Run:

```powershell
Get-ChildItem dashboard-ui/dist -Recurse -File |
  Select-String -Pattern "已启用","已禁用","加载失败","重载","下次启动后生效" -SimpleMatch
```

Expected: built assets contain the key Chinese plugin state/action labels.
...
- [ ] **Step 7: Commit final docs or cleanup changes**
...
Expected: commit succeeds if there are final uncommitted changes. If every prior
task committed its slice and this step has no diff, skip the commit.

- [ ] **Step 8: Optional push when requested**
